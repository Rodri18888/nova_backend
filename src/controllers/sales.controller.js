...
export async function createSale(req, res) {
  ...
  if (!['Efectivo', 'Tarjeta', 'Transferencia'].includes(paymentMethod)) return res.status(400).json({ error: 'Método de pago inválido' })
  ...
  const sale = await prisma.$transaction(async (tx) => {
    ...
    let calcSubtotal = 0, calcDiscount = 0
    for (const item of items) {
    ...
    const created = await tx.sale.create({
      data: {
        invoice, customerId: customerId || null, userId: req.user.id,
        paymentMethod, subtotal: calcSubtotal, tax: calcTax, total: calcTotal, discount: discount || 0,
        items: { create: items.map(i => ({ productId: i.productId, quantity: i.quantity, price: i.price, discount: i.discount || 0, subtotal: (i.price - (i.discount || 0)) * i.quantity })) },
      },
      include: { customer: true, user: true, items: { include: { product: true } } },
    })
    for (const item of items) {
      await tx.product.update({ where: { id: item.productId }, data: { stock: { decrement: item.quantity } } })
      await tx.inventoryMovement.create({ data: { productId: item.productId, type: 'salida', quantity: item.quantity, reason: `Venta ${invoice}` } })
      if (customerId) await tx.customer.update({ where: { id: customerId }, data: { points: { increment: Math.floor(item.price * item.quantity / 100) } } })
    }
    return created
  })
  res.json(sale)
